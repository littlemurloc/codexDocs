...
| formation_slot | string | 阵型锚点：front、skirmish、mid、back、support。 |
| formation_priority | int | 同锚点内的站位优先级，数值越高越优先靠前。 |
| formation_move_rule | string | 行军阶段移动规则，例如 follow_team_speed、hold_backline、charger_after_engage。 |
| combat_move_speed | float | 接敌后用于短距离调整、追击或突进的战斗移动速度。 |
| can_overtake_frontline | bool | 是否允许在行军阶段越过前排，默认 false；只给明确机制敌人开启。 |
...
| formation_slot | string | 阵型锚点：front、skirmish、mid、back、support。 |
| formation_priority | int | 同锚点内的站位优先级，数值越高越优先靠前。 |
| formation_move_rule | string | 行军阶段移动规则，例如 follow_team_speed、hold_backline、advance_with_front。 |
| combat_move_speed | float | 接敌后用于短距离调整、追击或突进的战斗移动速度。 |
| can_overtake_frontline | bool | 是否允许在行军阶段越过前排，默认 false；特殊武将或技能机制才开启。 |
