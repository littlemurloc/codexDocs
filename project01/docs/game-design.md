...
《三国斗阵》是一款面向网易 Y3 制作的单人 PVE 自动推线斗蛐蛐游戏。玩家在开局确定参战队伍，并为核心角色从 6 个技能中选择 4 个出战技能；战斗阶段双方队伍沿唯一主路线自动推进、索敌、释放技能。每波战斗结束后进入备战状态，玩家预览下一波敌人，并从 3 个随机战备能力中选择 1 个作为本局持续成长。胜负由队伍全灭判定。

核心体验：

- 阵容构筑：武将阵营、职业、技能装配与出兵顺序。
- 战备抉择：波次后预览下一波敌人，从 3 个本局继承能力中选择 1 个。
- 技能构筑：每名角色 6 选 4，形成同职业内的战斗风格分支。
...
| General Rules | 项目定位、单局循环、战斗规则、阵型推进、胜负判定、MVP边界 | [general-rules.md](design/general-rules.md) |
...
- 战场：唯一主路线，不做支路、绕路、多线推进。
- 阵型：行军阶段按队伍阵型整体推进，不依赖 Y3 原生阵型接口；敏将等高速职业在接敌后通过短突进、换目标和追击体现机动性。
...
2. 进入 Y3 策划落表和原型制作。
3. 首轮原型优先验证单线路径战斗、战备 3 选 1、奖励结算和 2 人/4 人差距。
