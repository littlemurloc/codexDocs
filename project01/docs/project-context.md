...
- 三国幻想题材。
- 单人 PVE 挑战。
- 上中下 3 路战斗，每方各有 3x3 共 9 个战前站位格。
- 波次后备战抉择，3 选 1 Roguelike 能力成长。
- 每名武将 6 个技能，战前携带 4 个。
- 主动技能默认自动释放；备战阶段可关闭某名已上阵英雄的主动技能自动释放，使其进入战斗 HUD 技能卡牌池由玩家手动释放。
- 装备围绕关键词放大角色当前流派。
- 阵营/职业羁绊提供阵容方向。
...
- 战场站位：我方 9 格在左，敌方 9 格在右并镜像摆放。我方格位从左到右、从上到下表示为 `741 / 852 / 963`，敌方按镜像规则对应。
- 选敌：战斗开始后单位优先攻击对应格位所在路线的敌方单位；若对应路线没有敌方单位，再按行规则选择次级目标。
- 站位被动：部分英雄被动技能受所处格位影响，放入指定格位会触发或改变被动效果。
- 技能卡牌：关闭自动释放的英雄主动技能进入手动技能卡牌池；当前上限为每轮抽取 4 张，使用完后再按规则重新抽取。冷却中、系统锁定或死亡英雄所属技能不会被抽取。
