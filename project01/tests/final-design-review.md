...
状态说明：本文档为历史终审记录，用于追溯进入 Y3 前的设计闭合判断。2026-07-08 后，战斗核心已由“唯一主路线”更新为“3 路 3x3 站位战斗 + 技能卡牌池”。当前 Y3 实装对齐、验收用例和后续执行顺序，以 `docs/y3-implementation-alignment.md`、`docs/y3-design-consistency-review.md` 和 `tests/acceptance-checklist.md` 为准。
...
- 单人 PVE、3 路 3x3 站位战斗、队伍全灭胜负。
- “关卡 N - 难度 1-10 - 波次 N”的关卡结构。
- 每波后备战状态、下一波敌人预览、战备 3 选 1、本局继承。
- 24 名完整武将池候选，每名 6 技能、战前携带 4 技能；主动技能支持自动释放或进入手动技能卡牌池。
...
| 3 路站位 vs 突进/切后 | 新口径需实测 | 突进和切后只改变短距离表现或目标优先级，不破坏 3 路格位读性。 |
...
| P1 | 3 路站位选敌是否会卡目标或无限索敌 | 用空行、单路密集、召唤、切后、Boss 波次专项测试。 |
| P1 | 技能卡牌池抽取、冷却和锁定是否稳定 | 测试自动开关、4 张上限、冷却中不抽、死亡英雄技能锁定。 |
...
2. 接入 8 名最低可测武将、基础技能装配、3 路 3x3 站位选敌和技能卡牌池。
3. 接入战备 3 选 1 和下一波预览。
